...
bool SurfaceFlinger::doComposeS3D(const sp<const DisplayDevice>& hw, const Region& dirty) {
    ...
    if (display != nullptr) {
        ...
        } else if (flags & HWC_IS_S3D_LAYER_TAB) {
        ...
        ALOGE("%s(%d): invalid display", __func__, __LINE__);
    }
    return result;
}
#endif

ssize_t SurfaceFlinger::registerVirtualDisplay(const sp<ANativeWindow>& nativeWindow, const String8& info) {
    return mMessageWDT.registerVirtualDisplay(nativeWindow, info);
}

ssize_t SurfaceFlinger::unregisterVirtualDisplay(const sp<ANativeWindow>& nativeWindow) {
    return mMessageWDT.unregisterVirtualDisplay(nativeWindow);
}
