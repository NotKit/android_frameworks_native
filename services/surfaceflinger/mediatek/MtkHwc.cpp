...
#ifdef USE_HWC2
void MtkHwc::createHwcValidateData(const DefaultKeyedVector< wp<IBinder>, sp<DisplayDevice> >& /*displays*/)
{
    auto& val_displays = mHwcValidateData.displays;
    auto& layers = mHwcValidateData.layers;

    layers.clear();

    if (val_displays.size() != 3)
        val_displays.resize(3);

    for (size_t i = 0; i < val_displays.size(); ++i)
    {
        auto& display = val_displays[i];
        display.id = i;
        display.flags = 0;
    }
}

HwcValidateData& MtkHwc::getHwcValidateData()
{
    return mHwcValidateData;
}
#endif
